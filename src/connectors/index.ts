...
//   (actions) => new EIP1193({ actions, provider: new Fortmatic(FORTMATIC_KEY).getProvider(), onError })
// )
...
export function useConnectors(selectedWallet: Wallet | undefined) {
  return useMemo(() => {
    ...
    // why pushing wallets seperately ? to get the selected one at the index 0 position?
    connectors.push(
